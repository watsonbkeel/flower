...
function careValidity() {
  ...
  return `<p role="status">${esc(validity.statusText)}</p>${validity.validUntilText ? `<p class="muted">有效期至：${esc(validity.validUntilText)}</p>` : ''}<button data-action="confirm-care" ${validity.canConfirm ? '' : 'disabled'}>确认养护卡</button>`;
}
