...
class USBCamera:
    def __init__(self, path, max_bytes=10 * 1024 * 1024, warmup_frames=5,
                 min_brightness=25, min_sharpness=40):
    ...
    def capture(self):
        ...
        result = subprocess.run(["python3", "-m", "flower_pi.camera.usb_camera", self.path,
                                 str(self.warmup_frames), str(self.min_brightness), str(self.min_sharpness)],
...
def main():
    ...
    try:
        ...
        if gray.mean() < float(brightness) or cv2.Laplacian(gray, cv2.CV_64F).var() < float(sharpness):
            raise ValueError("CAMERA_QUALITY_FAILED")
