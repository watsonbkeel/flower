        ...
        for provider in (self.primary, self.wired):
            ...
            try:
                ...
                if value.status == "READY" and value.observed_at:
                    ...
                    if 0 <= age <= self.stale_seconds and value.temperature_c is not None and value.humidity_pct is not None:
                        if math.isfinite(value.temperature_c) and -40 <= value.temperature_c <= 85 and 0 <= value.humidity_pct <= 100:
                            return value
...
class SHT30:
    ...
    def read(self):
        return AirReading("READY", self.sensor.temperature, self.sensor.relative_humidity,
                          datetime.now(timezone.utc), "sht30")
    ...
    def advertisement(self, device, advertisement):
        ...
        for uuid, data in advertisement.service_data.items():
            if uuid.startswith("0000181a") and len(data) == 13:
                ...
                self.latest = AirReading("READY", temperature, data[8],
                                          datetime.now(timezone.utc), "xiaomi_atc1441")
                return
            if uuid.startswith("0000fe95") and len(data) >= 5:
                encrypted = bool(int.from_bytes(data[:2], "little") & 0x08)
                self.latest = AirReading("BINDKEY_REQUIRED" if encrypted and not self.bindkey_configured
                                         else "UNSUPPORTED_MODEL", provider="xiaomi_ble")
                return
