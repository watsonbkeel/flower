...
@pytest.fixture
def state():
    return DeviceState(operating_mode="FULL", activity="IDLE", water_level_ok=True,
...
@pytest.mark.parametrize("source", ["cloud_auto", "user_manual", "local_fallback", "maintenance_test"])
...
@pytest.mark.parametrize("change", [
    {"activity": "CAPTURING"}, {"activity": "WATERING"},
    {"water_level_ok": False}, {"water_level_ok": None}, {"soil_pct": None},
    {"soil_pct": float("nan")}, {"time_trusted": False}, {"calibration_valid": False},
    {"profile_valid": False}, {"fault_codes": ("DISK_ERROR",)},
    {"fault_codes": ("PUMP_DRIVER_SUSPECT",)}, {"fault_codes": ("UNKNOWN_FAULT",)},
])
...
def test_sources_and_noncritical_faults(state):
    ...
    assert can_dispense(replace(state, fault_codes=("BLE_STALE", "CAMERA_UNAVAILABLE")), "cloud_auto").allowed
...
def test_calibration_uses_mean_and_rejects_invalid_measurements():
    ...
    with pytest.raises(ValueError):
        Calibration(flow_ml_sec=float("nan"), afterdrip_mean_ml=0, afterdrip_max_ml=0,
                    min_run_sec=1, adc_dry=20000, adc_wet=10000, insert_depth_mark="4cm", calibrated_at=NOW)
...
def test_unknown_timestamp_conservatively_ages_from_first_trusted_recovery(tmp_path):
    ...
    ledger.connection.execute("UPDATE water_ledger SET trusted_wall_time_utc=NULL, server_issued_at_utc=NULL")
...
def test_quota_does_not_drop_on_unverified_cloud_reconciliation(tmp_path):
    ...
    with pytest.raises(ValueError):
        ledger.reserve("two", "user_manual", 40, NOW + timedelta(hours=7), "boot", 20,
                       limit_ml=120, interval_hours=6)
    with pytest.raises(ValueError):
...
def test_reservation_is_atomic_between_connections(tmp_path):
    ...
    def reserve(number):
        ledger = Ledger(path)
        try:
            ledger.reserve(str(number), "user_manual", 60, NOW, "boot", 0,
                           limit_ml=120, interval_hours=6)
            return True
...
def make_policy(**changes):
    values = dict(policy_version=1, profile_version=1, generated_at=NOW,
                  valid_until=NOW + timedelta(days=7), trigger_soil_below_pct=28,
                  min_interval_hours=12, pulse_ml=10, max_pulses=1, max_session_ml=10,
                  max_24h_ml=60, timezone="Asia/Shanghai",
                  allowed_windows_local=[["06:00", "09:00"], ["17:00", "20:00"]],
                  profile_hash="sha256:" + "a" * 64)
    return Policy(**(values | changes))


@pytest.mark.parametrize("change", [{"timezone": "Invalid/Zone"}, {"timezone": ""},
    {"max_pulses": 2}, {"min_interval_hours": 2},
    {"allowed_windows_local": [["23:00", "01:00"]]},
    {"valid_until": NOW + timedelta(days=8)}, {"pulse_ml": float("nan")}])
...
def test_operating_mode_hysteresis(state):
    ...
    assert tracker.update(state, now_mono=400, cloud_age=301, policy_valid=True) == "LOCAL_CONSERVATIVE"
