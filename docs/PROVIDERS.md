...
- `PROVIDER_MODE=http` 的识别、检索来源、天气和通知回执必须各自声明 `source_type=real`；检索中只要有一个 Mock/演示来源就拒绝整批结果，HTTP 通知必须同时是 `status=sent`。无真实来源的网关响应不自动改标为真实，也不把 Mock 通知记为已送达。缺真实服务时继续使用显式标注的 Mock 模式。
